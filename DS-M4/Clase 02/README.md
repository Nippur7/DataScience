...
### 19) Imagenes:
1) $ sudo docker image ls (ver las imágenes que tengo localmente)
$ sudo docker pull ubuntu:20.04 (bajo la imagen de ubuntu con una versión específica)
...
RUN cat /hola.txt
##fin##
4) $ sudo docker build -t ubuntu:latest . (creo una imagen con el contexto de build <directorio>)
$ sudo docker run -it ubuntu:latest (corro el contenedor con la nueva imagen)
$ sudo docker login (me logueo en docker hub)
$ sudo docker tag ubuntu:latest miusuario/ubuntu:latest (cambio el tag para poder subirla a mi docker hub)
5) $ sudo docker push miusuario/ubuntu:latest (publico la imagen a mi docker hub)
