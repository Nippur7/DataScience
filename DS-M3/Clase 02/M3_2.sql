...
UPDATE `empleado` SET Salario2 = Salario;
...
ALTER TABLE `sucursal` 	ADD `Latitud1` DECIMAL(13,10) NOT NULL DEFAULT '0' AFTER `Longitud`, 
						ADD `Longitud1` DECIMAL(13,10) NOT NULL DEFAULT '0' AFTER `Latitud1`;
UPDATE `sucursal` SET Latitud1 = REPLACE(Latitud,',','.');
UPDATE `sucursal` SET Longitud1 = REPLACE(Longitud,',','.');
...
select count(*) from venta where Precio = '' or Cantidad = '';
...
UNION
SELECT DISTINCT Localidad, Provincia, Localidad, Provincia, 0 FROM proveedor where Localidad = 'Avellaneda'
ORDER BY 2, 1;
...
UNION
SELECT DISTINCT Localidad, Provincia, Localidad, Provincia, 0 FROM proveedor
ORDER BY 2, 1;
...
UPDATE proveedor p JOIN aux_localidad a
	ON (p.Provincia = a.Provincia_Original AND p.Localidad = a.Localidad_Original)
